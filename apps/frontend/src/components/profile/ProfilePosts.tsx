...
import { Images, Newspaper, Route, TrendingUp } from "lucide-react";
...
function ProfilePosts({ username, name, isOwnProfile }: { username: string; name: string; isOwnProfile: boolean }) {
  ...
  const [tab, setTab] = useState<ProfileTab>(requestedTab === "journey" || requestedTab === "photos" ? requestedTab : "posts");
  ...
  useEffect(() => {
    if (tab !== "journey" || logs !== null) return;
    ...
    <section>
      ...
      {tab === "journey" ? (
        <div className="space-y-6">
          <JourneyTimeline username={username} firstName={firstName} isOwnProfile={isOwnProfile} />
          <section>
            <h3 className="mb-2 px-1 text-xs font-quick font-bold uppercase tracking-wider text-chblack/45">Progress logs</h3>
            {logs === null ? (
