import { getHobbyColor, getHobbyText, getHobbyVoice } from "@/lib/hobbyTheme";
...
function HiveEmptyState({ hobby }: { hobby: string }) {
  ...
  const voice = getHobbyVoice(hobby);
  return (
    <div className="rounded-2xl border border-dashed border-chblack/15 p-10 text-center">
      ...
      <p className="mt-3 font-bnt text-3xl" style={{ color }}>
        {voice.emptyTitle.toUpperCase()}
      </p>
      <p className="mx-auto mt-1 max-w-sm text-sm text-chblack/60">{voice.emptyBody}</p>
    </div>
  );
}
